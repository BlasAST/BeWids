...
class ContenedorMensajes extends Component
{
    ...
    public $mensajesParticipanteActual;
    public $mensajesParticipantesExternos;
    public $prueba;
    public function buscarMensajes()
    {
        ...
        if ($this->participanteSeleccionado) {
            $this->mensajes = Mensaje::where('id_portal', FacadesSession::get('portal')->id)->where('conversacion_id', $this->conversacionSeleccionada->id)->where('emisor', $this->participanteActual->nombre_en_portal)->where('receptor', $this->participanteSeleccionado->nombre_en_portal)
                ->orWhere('id_portal', FacadesSession::get('portal')->id)->where('conversacion_id', $this->conversacionSeleccionada->id)->where('emisor', $this->participanteSeleccionado->nombre_en_portal)->where('receptor', $this->participanteActual->nombre_en_portal)->first();
            ...
            $this->mensajes = Mensaje::where('id_portal', FacadesSession::get('portal')->id)->where('conversacion_id', $this->conversacionSeleccionada->id)->where('participantesGroup','LIKE','%"'.$this->participanteActual->nombre_en_portal.'"%')->first();
            // whereRaw("JSON_CONTAINS(participantesGroup,?)",['{"emisor": "' .$this->participantesSeleccionados.'"}'])->first();
            // where('participantesGroup', json_encode($this->participantesSeleccionados))->first();
            if ($this->mensajes == null) {
                $mensajesConversacion = new Mensaje();
                $mensajesConversacion->id_portal = FacadesSession::get('portal')->id;
                $mensajesConversacion->conversacion_id = $this->conversacionSeleccionada->id;
    ...
    public function enviarMensaje()
    {
        if ($this->mensajeEnviado != null) {
            ...
            if ($this->mensajes->body == NULL) {
                $updateBody = [];
            } else {
                $updateBody = json_decode($this->mensajes->body, true);
            }
            $updateBody[] = $enviando;
